...
impl Space {
    fn empty_before() -> Space { Self { loc: SpaceLoc::Before, value: SpaceValue::None } }
    fn empty_after() -> Space { Self { loc: SpaceLoc::After, value: SpaceValue::None } }
...
impl fmt::Display for Space {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.value {
            ...
            SpaceValue::Indent { level, alignment, is_tab: false } => {
                write!(
                    f,
                    "\n{}",
            ...
            SpaceValue::Indent { level, is_tab: true, .. } => {
...
fn is_ws(token: &SyntaxToken) -> bool { token.kind() == TomlKind::Whitespace }
